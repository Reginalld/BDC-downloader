...
ALL_SENTINEL_TILES = {
                    tile for tiles_list in TILES_SENTINEL_POR_UF.values()
...
class DownloadRequest(BaseModel):
    ...
    def validate_tile_id(cls, v: Optional[str]) -> Optional[str]:
        ...
        if v_up not in ALL_VALID_UFS and v_up not in ALL_VALID_TILES:
            raise ValueError(
                f"ID de localização '{v_up}' "
                "inválido ou sem tiles disponíveis."
                )
    ...
    def validate_location_input(self):
        ...
        if has_tile_id and (self.lat or self.lon):
            raise ValueError(
                "Informe apenas 'tile_id' ou o par de coordenadas (lat/lon),"
                " não ambos."
                )
        ...
        if not has_tile_id and not has_lat_lon:
            raise ValueError(
                "É necessário fornecer 'tile_id' "
                "ou um par de coordenadas (lat/lon)."
                )
    ...
    def validate_location_and_satellite_compatibility(self):
        # Validador que checa a compatibilidade entre o ID e o satélite
